...
from market.models import PostSell, Seller, Bidder


def save_bid(request):
    context = dict()
    context['product_list'] = PostSell.objects.get(id=request.POST.get('product_id'))
    ...
    if request.method == 'POST':
        if int(request.POST.get('minimum_price')) > int(request.POST.get('bid_amount')):
            ...
            x = Bidder.objects.filter(product_id=PostSell.objects.get(id=request.POST.get('product_id'))).values(
	            'user_name')
            a = 0
            for item in x:
                if item['user_name'] == request.user.id:
                    y = Bidder.objects.get(user_name=request.user.id, product_id=PostSell.objects.get(
	                    id=request.POST.get('product_id')))
                    y.bid_amount = int(request.POST.get('bid_amount'))
                    y.save()
                    a = 1
            if not a:
                obj = Bidder(user_name=request.user, product_id=PostSell.objects.get(id=request.POST.get('product_id')),
